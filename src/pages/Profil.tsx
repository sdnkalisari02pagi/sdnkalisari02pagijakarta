...
export default function Profil() {
  ...
  useEffect(() => { if (tabParam && VALID_TABS.includes(tabParam)) setActiveTab(tabParam); }, [tabParam]);

  const jabatanOptions = useMemo(() => [...new Set(data.pegawai.map(p => p.jabatan))], [data.pegawai]);

  const pegawaiFiltered = useMemo(() => data.pegawai.filter(p => {
  ...
  const siswaFiltered = useMemo(() => data.siswa.filter(s => s.kelas.toLowerCase().includes(sSearch.toLowerCase())), [data.siswa, sSearch]);
      ...
      <div className="container mx-auto px-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="max-w-6xl mx-auto">
          ...
          <TabsContent value="sejarah" className="mt-6">
            ...
            <div className="text-muted-foreground leading-relaxed"><LinkedText text={tr(data.profil.sejarah, lang)} /></div>
          </TabsContent>
          ...
          <TabsContent value="visimisi" className="mt-6 space-y-6">
            <Card><CardContent className="pt-6">
              ...
              <div className="text-muted-foreground"><LinkedText text={tr(data.profil.visi, lang)} /></div>
            </CardContent></Card>
            <Card><CardContent className="pt-6">
              <h3 className="font-semibold text-lg text-foreground mb-2">{t('profil_misi')}</h3>
              <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                {data.profil.misi.map((m, i) => <li key={i}><LinkedText text={tr(m, lang)} /></li>)}
              </ol>
            </CardContent></Card>
            <Card><CardContent className="pt-6">
              <h3 className="font-semibold text-lg text-foreground mb-2">{t('profil_tujuan')}</h3>
              <div className="text-muted-foreground"><LinkedText text={tr(data.profil.tujuan, lang)} /></div>
            </CardContent></Card>
          </TabsContent>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              <FilterSidebar>
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <Input placeholder={t('search_nama')} value={pSearch} onChange={e => { setPSearch(e.target.value); setPPage(1); }} className="pl-10" />
                  </div>
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">{t('filter_jabatan')}</Label>
                  <Select value={pFilterJabatan} onValueChange={v => { setPFilterJabatan(v); setPPage(1); }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                  ...
                  {pegawaiPaged.length === 0 && <p className="col-span-full text-center text-muted-foreground py-8">{t('no_data')}</p>}
                </div>
                <PaginationBar total={pegawaiFiltered.length} page={pPage} perPage={pPerPage} onPageChange={setPPage} onPerPageChange={pp => { setPPerPage(pp); setPPage(1); }} options={[10, 20, 30, 'all']} />
              </div>
            </div>

            <Dialog open={!!selectedPegawai} onOpenChange={(open) => !open && setSelectedPegawai(null)}>
              <DialogContent className="sm:max-w-md">
                <DialogHeader><DialogTitle className="text-center">{selectedPegawai?.nama}</DialogTitle></DialogHeader>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              <FilterSidebar>
                <div>
                  ...
                  <div className="relative mt-1">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <Input placeholder={t('search_kelas')} value={sSearch} onChange={e => { setSSearch(e.target.value); setSPage(1); }} className="pl-10" />
                  </div>
                </div>
                <div className="rounded-lg bg-primary/5 p-3 border border-primary/20">
                  <p className="text-xs text-muted-foreground">{t('total_siswa')}</p>
                  <p className="text-2xl font-bold text-primary flex items-center gap-2"><GraduationCap className="w-5 h-5" /> {totalSiswa}</p>
                </div>
              </FilterSidebar>
                ...
                <div className="rounded-lg border bg-background overflow-hidden">
                  <table className="w-full">
                    <thead className="bg-muted/50">
                      <tr>
                        <th className="text-left px-4 py-3 font-semibold text-sm">{t('kelas')}</th>
                        <th className="text-right px-4 py-3 font-semibold text-sm">{t('jumlah_siswa')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {siswaPaged.map(s => (
                      ...
                      {siswaPaged.length === 0 && <tr><td colSpan={2} className="text-center text-muted-foreground py-6">{t('no_data')}</td></tr>}
                    </tbody>
                  </table>
                </div>
                <PaginationBar total={siswaFiltered.length} page={sPage} perPage={sPerPage} onPageChange={setSPage} onPerPageChange={pp => { setSPerPage(pp); setSPage(1); }} options={[10, 20, 30, 'all']} />
              </div>
            </div>
          </TabsContent>
        </Tabs>
